...
function findLongestWord(words) {
 ...
 // const uniquifyArray = ar => [...new Set(ar)];
...
//  const doesWordExist = (ar, word) => ar.includes(word)
...
// alternative : comparing each entry with word and incrementing a counter if there is match
...
const greatestProduct = m => {
  let res = 0;
  let horizontal = 0;
  let vertical = 0;
  for (let j = 0; j < 20; j ++) {
    for (let i = 0; i < 17; i ++) {
      horizontal = m[j][i] * m[j][i + 1] * m[j][i + 2] * m[j][i + 3];
